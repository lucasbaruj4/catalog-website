    ...
    <section v-if="view === 'add' || view === 'edit'">
      <q-form @submit="onSubmit">
        <q-card>
          <q-card-section>
            <div class="text-h6">
              {{ view === "add" ? "Agregar Categoría" : "Editar Categoría" }}
            </div>
...
export default {
  ...
  setup() {
    ...
    const onSubmit = () => {
      if (view.value === "add") {
        axios
          .post("http://127.0.0.1:5000/api/categorias", form.value)
          .then(() => {
            onRequest();
            view.value = "list";
          })
        ...
        axios
          .put(
            `http://127.0.0.1:5000/api/categoria/${form.value.id}`,
            form.value
          )
          .then(() => {
            onRequest();
    ...
    const onDelete = (id) => {
      if (confirm("¿Estás seguro de que deseas eliminar esta categoría?")) {
        axios
          .delete(`http://127.0.0.1:5000/api/categoria/${id}`)
          .then(() => {
            onRequest();
          })
          .catch((error) => console.error(error));
    ...
    const onRequest = () => {
      ...
      axios
        ...
        .then((response) => {
          ...
          rows.value = response.data.categorias || [];
        })
        .catch((error) => {
          console.log(error);
        })
